...
export function applyWorkspaceCommand(source: AppState, role: Role, action: string, payload: unknown, at: string): CommandResult {
  ...
  switch (action) {
    ...
    case "capture_inbox": {
      const input = parse(captureSchema.extend({ text: z.string().trim().min(2), fileName: z.string().trim().max(255).optional() }), payload); const captureId = id(); state.inbox.unshift({ id: captureId, type: input.type, raw: input.text, fileName: input.fileName, status: "needs_review", createdAt: at, proposal: makeProposal(input.text) }); return { state, message: "Saved to Inbox. Your original capture is safe on the server.", id: captureId };
    }
    case "accept_proposal": {
      const input = parse(z.object({ id: identifier, mode: z.enum(["create","attach"]), sectionId: identifier.optional(), itemType: z.enum(["note","action","idea","product","decision","link","person","place","event","reservation","file","checklist"]).optional() }), payload); const capture = state.inbox.find((item) => item.id === input.id); if (!capture?.proposal) throw new Error("Capture proposal not found."); const targetId = input.mode === "attach" ? capture.proposal.relatedThingId : undefined;
