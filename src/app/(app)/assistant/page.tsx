...
export default function AssistantPage() {
  ...
  return <div className="page calm-assistant-page">
    ...
    <section className="assistant-today-section"><h2>Working on</h2><div className="do-next-list">{doNext.map((thing) => <article key={thing.id}><Link href={`/things/${thing.id}`}><strong>{thing.title}</strong><p>{thing.assistantNextAction}</p>{thing.dates[0] ? <small>{daysUntil(thing.dates[0].start, renderedAt)}</small> : null}</Link><div><button disabled={role !== "assistant"} onClick={() => open(thing, "movement")}>Add update</button><details><summary aria-label={`More actions for ${thing.title}`}><MoreHorizontal size={16} /></summary><div><button disabled={role !== "assistant"} onClick={() => open(thing, "follow-up")}>Follow up</button><button disabled={role !== "assistant"} onClick={() => open(thing, "link")}>Add link</button><button disabled={role !== "assistant"} onClick={() => open(thing, "option")}>Add option</button><button disabled={role !== "assistant"} onClick={() => open(thing, "approval")}>Ask Jerry</button></div></details></div></article>)}</div></section>
