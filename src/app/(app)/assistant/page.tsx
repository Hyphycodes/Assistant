...
export default function AssistantPage() {
  ...
  return <div className="page assistant-page">
    ...
    <div className="assistant-grid">
      ...
      <section className="assistant-section wide"><div className="section-heading compact"><div><p className="eyebrow">Follow-through</p><h2>Waiting on others</h2></div><UsersRound size={18} /></div>{waiting.length ? <div className="waiting-strip">{waiting.map((thing) => <article key={thing.id}><MessageSquareText size={17} /><span><Link href={`/things/${thing.id}`}><strong>{thing.title}</strong></Link><p>Waiting on {thing.waitingOn ?? "a response"}. {thing.assistantNextAction}</p></span><button disabled={role !== "assistant"} onClick={() => open(thing, "follow-up")}><Send size={14} />Prepare follow-up</button></article>)}</div> : <div className="calm-empty"><Check size={17} /><p>No follow-up is overdue.</p></div>}</section>
