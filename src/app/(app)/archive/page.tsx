...
export default function ArchivePage() {
  ...
  return (
    <div className="page archive-page">
      ...
      <section className="search-group"><div className="search-group-title"><PackageCheck size={16} /><h2>{needle ? "Things & context" : "Completed Things"}</h2><span>{results.length}</span></div>{results.map((thing) => <Link href={`/things/${thing.id}`} className="search-result" key={thing.id}><div><div className="result-heading"><strong>{thing.title}</strong><StatusBadge status={thing.status} /></div><p>{thing.summary}</p><small>{thing.category} · Last moved {formatDate(thing.lastMoved)}</small></div><ArrowRight size={16} /></Link>)}</section>
